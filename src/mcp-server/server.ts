...
const FINETUNE_FILE = path.join(DATA_DIR, 'finetune', 'prescription_finetune_dataset.jsonl');
...
async function initializeDataDir() {
  try {
    ...
    try {
      ...
      await fs.writeFile(PATIENTS_FILE, JSON.stringify(defaultPatients, null, 2));
    }
...
async function savePrescriptions(prescriptions: Prescription[]): Promise<void> {
  await fs.writeFile(PRESCRIPTIONS_FILE, JSON.stringify(prescriptions, null, 2));
}
...
async function generateAiPrescription(
  ...
  const matches = finetuneData.filter(data => {
    ...
    const symptomMatch = symptoms.some(s => 
      data.input.symptoms.some((ds: string) => ds.toLowerCase().includes(s.toLowerCase()))
    );
    const diagnosisMatch = diagnosis 
      ? data.input.preliminary_diagnosis && 
        data.input.preliminary_diagnosis.toLowerCase().includes(diagnosis.toLowerCase())
      : true;
  ...
  if (matches.length === 0) {
    throw new Error("No matching prescription found for the given symptoms and conditions");
  }
  ...
  return {
    tools: [
      ...
      },
      {
        ...
        description: 'Create a new prescription for a patient using AI analysis',
        inputSchema: {
          type: 'object',
          properties: {
            patientId: {
      ...
      },
      {
        ...
        description: 'Update an existing prescription with doctor modifications',
        inputSchema: {
          type: 'object',
          properties: {
            prescriptionId: {
  ...
  try {
    switch (name) {
      case 'get_patient_history': {
        ...
        if (!patient) {
          throw new McpError(ErrorCode.InvalidRequest, `Patient with ID ${patientId} not found`);
        }
      ...
      case 'create_prescription': {
        ...
        const aiResult = await generateAiPrescription(patient, symptoms, diagnosis);
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                patientHistory: {
                  totalPrescriptions: patientPrescriptions.length,
                  previousMedications: patientPrescriptions.flatMap(p => p.medications.map(m => m.name)),
      ...
      case 'update_prescription': {
        ...
        const feedbackFile = path.join(DATA_DIR, 'feedback', `${doctorId}_feedback.jsonl`);
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                updatedPrescription,
                feedbackStored: true,
                message: 'Prescription updated successfully and feedback stored for model improvement',
              }, null, 2),
            },
          ],
        };
